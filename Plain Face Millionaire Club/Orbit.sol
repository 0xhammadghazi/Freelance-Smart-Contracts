    ...
    */
    ...
    function updateReward(
        ...
        if (rewardGenerationStartTime != 0) {
            ...
            if (from != address(0)) {
                rewards[from] += getPendingReward(from, reward, nftBalanceFrom);
                uint256 timerFrom = lastUpdate[from];
                if (timerFrom != END) lastUpdate[from] = time;
            }
            if (to != address(0)) {
                rewards[to] += getPendingReward(to, reward, nftBalanceTo);
                uint256 timerTo = lastUpdate[to];
                if (timerTo != END) lastUpdate[to] = time;
            }
        }
    }
    ...
    function claimReward(address user) external {
        require(allowedAddresses[msg.sender], "Unauthorized caller");
        require(rewardStatus, "Reward claiming is turned off");
    ...
    function getTotalClaimable(
        ...
        if (rewardGenerationStartTime == 0) {
            ...
            return rewards[user] + getPendingReward(user, reward, nftBalance);
        }
    }
